#!/bin/bash
# The Publisher Routine (for creating content)

# --- HOW TO USE ---
# This script posts content to paip.ai. It's designed to be portable and
...
#
# Usage for a text-only post:
#   ./publisher.sh "This is my text-only post."
#
# Usage for a post with an image or video:
#   ./publisher.sh "Check out this cool image!" "/path/to/my/image.jpg"
...
if [[ -n "$MEDIA_FILE" ]]; then
    ...
    echo "  - Preparing a text-only post."
fi
...
if [[ $(echo "$POST_RESPONSE" | jq -r '.code') == "0" ]]; then
    echo "--- SUCCESS! Moment posted successfully. ---"
else
